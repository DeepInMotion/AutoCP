...
class Trainer:
    ...
    def trainer_eval_cp(self, epoch: int, student_id: int, ensemble=False) -> Union[tuple[ndarray, ndarray, ndarray],
        ...
        with torch.no_grad():
            ...
            metrics_cp = self.evaluate_cp(preds, labels, video_ids, prediction_threshold=0.5, subject=True)
            metrics_cp_win = self.evaluate_cp(preds, labels, video_ids, prediction_threshold=0.5, subject=False)
